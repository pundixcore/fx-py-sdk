    ...
    def _handle_response(response):
        ...
        try:
            ...
            if 'error' in res and res['error']:
                raise FxdexRPCException(response)
            ...
            raise FxdexRequestException('Invalid Response: %s' % response.text)
...
class AsyncHttpRpcClient(BaseHttpRpcClient):
    ...
    async def _handle_session_response(self, response):
        ...
        try:
            ...
            raise FxdexRequestException('Invalid Response: %s' % await response.text())
    ...
    async def get_validators(self, height: int):
        data = {
            'height': str(height),
            'page': '1',
            'per_page': '10'
